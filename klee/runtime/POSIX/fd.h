...
static const int MAX_LINKS = 5;

typedef struct {
    int refcnt;
    unsigned size;
    unsigned buf_size; // must be >= size
    char *contents;
    struct stat64 *stat;
    int mode;
    char** name;
...
typedef struct {      
  ...
  union {
      ...
      exe_disk_con_file_t *dcfile; // concrete disk file
  };
  enum {
      CONCRETE_FILE = 1,
      SYMBOLIC_FILE = 2
