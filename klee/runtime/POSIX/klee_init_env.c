...
usage: (klee_init_env) [options] [program arguments]\n\
  ...
  while (k < argc) {
    ...
    else if (__streq(argv[k], "--malloc-fail") || __streq(argv[k], "-malloc-fail")) {
      malloc_fail = 1;
      k++;
    }
	...
	__emit_error(msg);
    ...
    else if (__streq(argv[k], "--make-etc-symbolic") || __streq(argv[k], "-make-etc-symbolic")) {
      etc_symbolic = 1;
      k++;
    }
    else if (__streq(argv[k], "--disable-sym-filter") || __streq(argv[k], "-disable-sym-filter")) {
      sym_specific = 0;
      k++;
    }
    else if (__streq(argv[k], "--symbolic-size") || __streq(argv[k], "-symbolic-size")) {
        const char *msg = "--symbolic-size expects one integer argument <symbolic file size>";
      ...
      symbolic_size = __str_to_int(argv[k++], msg);
    }
    else {
  ...
  *argvPtr = final_argv;
  ...
  klee_init_fds(sym_files, sym_file_len, 
		...
		fd_fail, malloc_fail, etc_symbolic, symbolic_size, sym_specific);
}
