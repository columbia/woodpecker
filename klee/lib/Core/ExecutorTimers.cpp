...
public:
  TimerInfo(Timer *_timer, double _rate) 
    : timer(_timer),
      rate((long)_rate),
      nextFireTime((long)time(NULL) + rate) {}
  ~TimerInfo() { delete timer; }
};

void Executor::addTimer(Timer *timer, double rate) {
  timers.push_back(new TimerInfo(timer, (long)rate));
}
...
void Executor::processTimers(ExecutionState *current,
  ...
  if (ticks || dumpPTree || dumpStates) {
    ...
    if (dumpStates) {
      ...
      if (os) {
        for (std::set<ExecutionState*>::const_iterator it = states.begin(), 
          ...
          *os << "{";
          for (ExecutionState::threads_ty::iterator ithr = es->threads.begin();
              ithr != es->threads.end(); ithr++)
          {
            *os << "[";
            ThreadState &thr = ithr->second;
            ThreadState::stack_ty::iterator next = thr.stack.begin();
            ++next;
            ...
            *os << "{";
            ...
            uint64_t md2u = computeMinDistToUncovered(thr.pc,
    ...
    if (!timers.empty()) {
      long curTime = (long)time(NULL);  // Heming: would this be expensive? Invoked at each loop of run()?

      for (std::vector<TimerInfo*>::iterator it = timers.begin(), 
             ie = timers.end(); it != ie; ++it) {
        TimerInfo *ti = *it;
        
        if (curTime >= ti->nextFireTime) {
          ti->timer->run();
          ti->nextFireTime = curTime + ti->rate;
        }
      }
    }
