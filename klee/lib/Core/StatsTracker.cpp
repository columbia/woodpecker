...
extern cl::opt<bool> UsePathSlicer;
extern PathSlicer *pathSlicer;
...
StatsTracker::StatsTracker(Executor &_executor, std::string _objectFilename,
  ...
  for (std::vector<KFunction*>::iterator it = km->functions.begin(), 
    ...
    KFunction *kf = *it;
    kf->trackCoverage = UsePathSlicer?pathSlicer->isInternalFunction(kf->function):1;
    if (UsePathSlicer && kf->trackCoverage) {
      errs() << "StatsTracker trackCoverage: " << kf->function->getNameStr() << "\n";
    }
...
void StatsTracker::stepInstruction(ExecutionState &es, int tid) {
  if (OutputIStats) {
    ...
    Instruction *inst = es.getThread(tid)->pc->inst;
    const InstructionInfo &ii = *(es.getThread(tid)->pc)->info;
    StackFrame &sf = es.getThread(tid)->stack.back();
    theStatisticManager->setIndex(ii.id);
    if (UseCallPaths)
    {
        if (sf.callPathNode == NULL)
        {
            assert(0 && "NULL callPathNode!");
        }
...
void StatsTracker::framePushed(ExecutionState &es, StackFrame *parentFrame, int tid) {
  if (OutputIStats) {
    StackFrame &sf = es.getThread(tid)->stack.back();
...
void StatsTracker::updateStateStatistics(uint64_t addend) {
  for (std::set<ExecutionState*>::iterator it = executor.states.begin(),
    ...
    // XXX
    const InstructionInfo &ii = *state.lastThread()->pc->info;
    theStatisticManager->incrementIndexedValue(stats::states, ii.id, addend);
    if (UseCallPaths)
      // XXX
      state.lastThread()->stack.back().callPathNode->statistics.incrementValue(stats::states, addend);
  }
}
...
void StatsTracker::computeReachableUncovered() {
  ...
  for (std::set<ExecutionState*>::iterator it = executor.states.begin(),
    ...
    for (ExecutionState::threads_ty::iterator tit = es->threads.begin(),
            tie = es->threads.end(); tit != tie; tit++) {
      uint64_t currentFrameMinDist = 0;
      ThreadState &thr = tit->second;
      for (ThreadState::stack_ty::iterator sfIt = thr.stack.begin(),
          sf_ie = thr.stack.end(); sfIt != sf_ie; ++sfIt) {
        ThreadState::stack_ty::iterator next = sfIt + 1;
        KInstIterator kii;

        if (next==thr.stack.end()) {
          kii = thr.pc;
        } else {
          kii = next->caller;
          ++kii;
