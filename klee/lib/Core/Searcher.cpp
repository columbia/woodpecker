...
double WeightedRandomSearcher::getWeight(ExecutionState *es) {
  ...
  case InstCount: {
    uint64_t count = theStatisticManager->getIndexedValue(stats::instructions,
                                                          es->lastThread()->pc->info->id);
    double inv = 1. / std::max((uint64_t) 1, count);
    return inv * inv;
  }
  case CPInstCount: {
    StackFrame &sf = es->lastThread()->stack.back();
  ...
  case CoveringNew:
  case MinDistToUncovered: {
    uint64_t md2u = computeMinDistToUncovered(es->lastThread()->pc,
                                              es->lastThread()->stack.back().minDistToUncoveredOnReturn);
...
Instruction *BumpMergingSearcher::getMergePoint(ExecutionState &es) {  
  if (mergeFunction) {
    // XXX
    Instruction *i = es.lastThread()->pc->inst;
