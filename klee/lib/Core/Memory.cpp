...
#ifdef MEMORY_DEBUG
int MemoryObject::totalSize = 0;
#endif

MemoryObject::~MemoryObject() {
#ifdef MEMORY_DEBUG
  MemoryObject::totalSize -= size;
  std::cerr << "delete MemoryObject: " << size << " totalSize: " << MemoryObject::totalSize << std::endl;
#endif
...
#ifdef MEMORY_DEBUG
  totalSize += size;
  std::cerr << "new ObjectState: " << size << " total size: " << std::dec << totalSize << std::endl;
#endif
...
ObjectState::ObjectState(const ObjectState &os) 
  ...
  std::cerr << "parts size: ";
  if (concreteStore)
      std::cerr << sizeof(*concreteStore) << "/";
  else
      std::cerr << "0/";

  if (concreteMask)
      std::cerr << sizeof(*concreteMask) << "/";
  else
      std::cerr << "0/";
  if (flushMask)
      std::cerr << sizeof(*flushMask) << "/";
  else
      std::cerr << "0/";
  if (knownSymbolics)
      std::cerr << sizeof(*knownSymbolics) << "/";
  else
      std::cerr << "0/";
  std::cerr << sizeof(updates) << "/";
  std::cerr << "\n";
#endif
}
