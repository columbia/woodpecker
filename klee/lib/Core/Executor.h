...
private:
  ...
  /// Checkers shared by all paths
  CheckerMgr globalCheckers;

  /// The branches irrelevant to the slicing targets.
  //std::set<llvm::BranchInst *> rmBrs;
  ...
  bool willBlock(ExecutionState &state, int tid, std::set<int>& enable_set);
  void runNextInstruction(ExecutionState &state);
  
  void executeInstruction(ExecutionState &state, int tid, KInstruction *ki);
  ...
  void stepInstruction(ExecutionState &state, int tid);
  void updateStates(ExecutionState *current);
  void transferToBasicBlock(llvm::BasicBlock *dst, 
			    llvm::BasicBlock *src,
			    ThreadState &state);

  void emulateSync(ExecutionState &state,
                            ThreadState &thr,
                            KInstruction *target,
  ...
  ObjectState *bindObjectInState(ExecutionState &state, ThreadState *thr,
  ...
  bool callChecker(ExecutionState &newState, KInstruction *target, int tid);
  ...
  StatePair fork(ExecutionState &current, int tid, ref<Expr> condition, bool isInternal);
  ...
  Cell& getArgumentCell(ThreadState &state,
                        KFunction *kf,
                        unsigned index) {
    return state.stack.back().locals[kf->getArgRegister(index)];
  }

  Cell& getDestCell(ThreadState &state,
                    KInstruction *target) {
  ...
  void executeGetValue(ExecutionState &state, ThreadState &thr, ref<Expr> e, KInstruction *target);

  /// Get textual information regarding a memory address.
  std::string getAddressInfo(ExecutionState &state, ref<Expr> address) const;

  void terminateStateAtOnce(ExecutionState &state);
  ...
  //bool curBrCanDiscard(ExecutionState &state, int tid);
  bool curInstrIsInternal(KInstruction *kInstr);
  void printKLEECallStack(ExecutionState &state);
  void slicingAndPruneStates(ExecutionState &state);
