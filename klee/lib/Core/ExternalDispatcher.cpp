...
bool ExternalDispatcher::executeCall(Function *f, Instruction *i, uint64_t *args) {
  ...
  /*fprintf(stderr, "ExternalDispatcher::executeCall START (%s:%s:%s)\n",
    f->getNameStr().c_str(),
    i->getParent()->getParent()->getNameStr().c_str(),
    i->getParent()->getNameStr().c_str());//Heming: added for debugging external call.
  fflush(stderr);*/
  bool result = runProtectedCall(dispatcher, args);
  /*fprintf(stderr, "ExternalDispatcher::executeCall END result %d (%s:%s:%s)\n",
    result,
    f->getNameStr().c_str(),
    i->getParent()->getParent()->getNameStr().c_str(),
    i->getParent()->getNameStr().c_str());//Heming: added for debugging external call.
