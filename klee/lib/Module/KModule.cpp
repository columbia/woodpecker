...
void KModule::prepare(const Interpreter::ModuleOptions &opts,
  ...
  // change some functions to correct names
  // so that we use uClic instead of make external calls
  Function *f_torep = module->getFunction("__strdup");
  if (f_torep != NULL)
  {
      Function *f_target = module->getFunction("strdup");
      if (f_target == NULL)
      {
          f_torep->setName("strdup");
      } else {
          std::vector<CallSite> to_replace;
          for (Value::use_iterator ui = f_torep->use_begin(); ui != f_torep->use_end(); ui++)
          {
              if (Instruction *ins = dyn_cast<Instruction>(*ui)) {
                  CallSite cs(ins);
                  if (cs.getInstruction()) {
                      to_replace.push_back(cs);
                  }
              }
          }
          for (unsigned int i=0; i<to_replace.size(); i++)
              to_replace[i].setCalledFunction(f_target);
      }
  }
