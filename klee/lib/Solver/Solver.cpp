...
static void runAndGetCex(::VC vc, STPBuilder *builder, ::VCExpr q,
  ...
  // Heming: use the same way to calculate stpTimeout as in Executor.cpp.
  double stpTimeout = (MaxSTPTime != 0 && MaxInstructionTime != 0)
      ? std::min(MaxSTPTime,MaxInstructionTime)
      : std::max(MaxSTPTime,MaxInstructionTime);
  int result = vc_query_with_timeout(vc, q, stpTimeout*1000.0);
  const int SOLVER_TIMEOUT = 3;
  if (result == SOLVER_TIMEOUT)
      fprintf(stderr, "WARNING! SOLVER TIMEOUT!\n");
  hasSolution = !result;
...
static bool runAndGetCexForked(::VC vc, 
  ...
  if (pid == 0) {
    ...
    unsigned res = vc_query_with_timeout(vc, q, timeout*1000.0);
    if (!res) {
      for (std::vector<const Array*>::const_iterator
             it = objects.begin(), ie = objects.end(); it != ie; ++it) {
        const Array *array = *it;
