...
std::string KleeHandler::getOutputDirectory() {
  fprintf(stderr, "KleeHandler::getOutputDirectory %s\n", m_outputDirectory);
  return std::string(m_outputDirectory);
}
...
static void parseArguments(int argc, char **argv) {
  ...
  cl::ParseCommandLineOptions(numArgs, (char**)((long)argArray), " klee\n");
  delete[] argArray;
}
...
static llvm::Module *linkWithUclibc(llvm::Module *mainModule) {
  ...
  if (WithPOSIXRuntime) {
    ...
    mainModule->getOrInsertFunction("fwrite",
                                    ...
                                    Type::getInt32Ty(getGlobalContext()),
                                    NULL);
    mainModule->getOrInsertFunction("strncpy",
                                    PointerType::getUnqual(i8Ty),
                                    PointerType::getUnqual(i8Ty),
                                    Type::getInt32Ty(getGlobalContext()),
                                    PointerType::getUnqual(i8Ty),
    ...
    mainModule->getOrInsertFunction("__fputc_unlocked",
                                    ...
                                    PointerType::getUnqual(i8Ty),
                                    NULL);
    mainModule->getOrInsertFunction("strstr",
                                    PointerType::getUnqual(i8Ty),
                                    PointerType::getUnqual(i8Ty),
                                    PointerType::getUnqual(i8Ty),
                                    NULL);
...
/* This function must be called before uclibc is linked in to the main module. */
void initPathSlicer(Module &M, std::string path) {
  pathSlicer = new PathSlicer();
  pathSlicer->readModulePath(path);
...
int main(int argc, char **argv, char **envp) {  
  ...
  if (UsePathSlicer)
    initPathSlicer(*mainModule, InputFile);
  ...
  if (UsePathSlicer)
    pathSlicer->checkInstrMapConsistency("check after linkWithUclibc");
  ...
  if (UsePathSlicer) {
    pathSlicer->initOutputDir(handler->getOutputDirectory());
  }
  ...
  if (UsePathSlicer)
    pathSlicer->checkInstrMapConsistency("check after setModule");
