...
char **add_quote_to_args(int argc, char *argv[]) {
  char **retArgs = (char **)malloc(argc*sizeof(char *)+1);  // The 1 is for execv() convention.
  assert(retArgs);
  int i;
  for (i = 0; i < argc; i++) {
    retArgs[i] = (char *)malloc(strlen(argv[i])+2+1); // 2 are the two quotes, 1 is the '\0'.
    assert(retArgs[i]);
    sprintf(retArgs[i], "\"%s\"", argv[i]);
    fprintf(stderr, "retArgs[%d]: (%s)\n", i, retArgs[i]);
  }
  retArgs[argc] = NULL;
  return retArgs;
}
...
int main(int argc, char** argv) {
  ...
  for (idx = 2; idx != argc; ++idx) {
    ...
    prg_argv[0] = argv[1];
    klee_init_env(&prg_argc, &prg_argv);
    char **quoteArgs = prg_argv;//add_quote_to_args(prg_argc, prg_argv);
    ...
    if (pid < 0) {
      ...
      replay_create_files(&__exe_fs);
      run_monitored(executable, prg_argc, quoteArgs);
      _exit(0);
    } else {
...
void klee_notify_counter(int counter, const char *name) {}

int klee_sys_open(char* path, int flags, int mode) {
    return open(path, flags, mode);
}

int klee_sys_close(int fd) {
    return close(fd);
}
