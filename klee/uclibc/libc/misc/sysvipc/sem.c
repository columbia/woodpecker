...
#ifdef __NR_semctl
	// Heming: for 64 bit.
	return syscall(SYS_semctl, semnum, cmd | __IPC_64, arg.__pad);
//    return __semctl(semid, semnum, cmd | __IPC_64, arg.__pad);
#else
//    return __syscall_ipc(IPCOP_semctl, semid, semnum, cmd | __IPC_64, &arg);
    return syscall(SYS_ipc, IPCOP_semctl, semid, semnum, cmd | __IPC_64, &arg);
#endif
}
#endif
...
{
	return syscall(SYS_semget, key, nsems, semflg, NULL);
    //return syscall(SYS_ipc, IPCOP_semget, key, nsems, semflg, NULL);
}
#else
...
{
	return syscall(SYS_semop, semid, (int) nsops, 0, sops);
//    return __syscall_ipc(IPCOP_semop, semid, (int) nsops, 0, sops);
//return syscall(SYS_ipc, IPCOP_semop, semid, (int) nsops, 0, sops);
}
#else
