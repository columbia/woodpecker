...
# the ARGV[10], optional (default empty ""), last concrete argument.
...
$symbolicETC = ""; # Default, no symbolic etc.
if (scalar(@ARGV) > 8) {
...
$lastConcreteArg = "";
if (scalar(@ARGV) > 10) {
	$lastConcreteArg = @ARGV[10];
	print "Last concrete argument is $lastConcreteArg\n";
}
...
@excludeProgs = (
	...
	# touch in coreutils, calls yystype which is 448 bits, and KLEE creates an OrExpr, which cast to at 64bits concrete value.
	# This stp bug only shows up in real prune, not in mark prune, showing that we explored deeper code than orignal KLEE.
	"touch",
...
sub runKLEE {
	...
	"$progName --sym-files $symbolicFiles $symbolicETC --max-fail $maxFail ".
	" --sym-args $symbolicArgs $lastConcreteArg     2>&1 | tee $outFile";
	print `date`;
...
sub evalProgram {
	...
	if (($mode eq "both") || ($mode eq "mark-pruned-only")) {
		...
		`cp ../local.options .`;
		`cp ../bc2bdd.conf .`;
		`echo \"resultsdir=../bdd-eval-$progOrigName[0]\" >> bc2bdd.conf`;
		#`chmod -R 777 .`;
		$enablePathSlicer = 1;
		$markPrunedOnly = 1;
